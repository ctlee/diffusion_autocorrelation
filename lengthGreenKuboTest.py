...
fn = 'lengthGreenKuboCrossing0.txt'
crossing = 0 
...
def worker(args):
    ...
    #index = np.where(np.logical_and(vacr>=-1*threshold, vacr <=threshold))
    ...
    out_q.put("%0.2f %0.4e"%(args[2], Dvacr))
...
def plot():
    ...
    plt.savefig('Figures/lengthGreenKubo.png', dpi=1200, transparent=True)
    plt.savefig('Figures/lengthGreenKubo.eps', dpi=1200)
    plt.show()
