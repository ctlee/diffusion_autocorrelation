...
def worker(args):
    ...
    cuda.select_device(args[3]%2)
    prng = curand.PRNG(rndtype=curand.PRNG.MTGP32, seed=args[3])
    noises = np.empty(N)
    prng.normal(noises, 0, noise, size=N)
    cuda.close()
    forces = noises/dt
    print forces
    slice = 1
    facr = model.fftAutocovariance(forces[::slice])
...
def run():
    ...
    out_q = manager.Queue()
    pool = multiprocessing.Pool(maxtasksperchild=1, processes=7)
...
def plot():
    ...
    plt.savefig('Figures/lengthSFDT.png', dpi=1200, transparent=True)
    plt.savefig('Figures/lengthSFDT.eps', dpi=1200)
    plt.show()
