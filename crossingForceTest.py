...
def worker(args):
    ...
    if len(index):
        for crossing in np.arange(0,10,1):
            ...
            out_q.put("%d %0.4e"%(crossing, Dfacr))
    else:
        facrtrunc = facr
        Dfacr = 2*(kb*T)**2/(np.sum(facrtrunc)*dt*slice)*1e-16 # cm^2/s
        out_q.put("%d %0.4e %d"%(-1, Dfacr, args[2]))
...
def run():
    ...
    out_q = manager.Queue()
    pool = multiprocessing.Pool(maxtasksperchild=1, processes=9)
...
def plot():
    ...
    plt.savefig('Figures/crossingSFDT.png', dpi=1200, transparency=True)
    plt.savefig('Figures/crossingSFDT.eps', dpi=1200)
    plt.show()
