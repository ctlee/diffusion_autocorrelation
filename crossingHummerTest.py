#!/bin/env python2.7
"""
@description: Investigating the effect of cutoff on the Hummer estimator
...
def worker(args):
    ...
    if len(index):
        for crossing in np.arange(0,10,1):
            ...
            out_q.put("%d %0.4e"%(crossing, Dhummer))
    else:
        acrtrunc = pacr
        tau = np.sum(acrtrunc)*dt*slice
        Dhummer = np.var(traj[0] - np.mean(traj[0]))*math.pow(10,-16)/tau
        out_q.put("%d %0.4e"%(-1, Dhummer))
...
def plot():
    ...
    plt.savefig('Figures/crossingHummer.png', dpi=1200, transparency=True)
    plt.savefig('Figures/crossingHummer.eps', dpi=1200)
    plt.show()
