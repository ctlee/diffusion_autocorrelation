...
fn = 'lengthHummerAvgTestCrossing0.txt'
crossing = 0
...
def worker(args):
    ...
    slice = 1 
    dN = length/dt/10
    DhummerArray = np.zeros(10)
    for i in np.arange(0,10,1):
        pacr = model.fftAutocorrelation(traj[0][i*dN:(i+1)*dN:slice])
        index = np.where(np.diff(np.sign(pacr)))[0]

        if len(index):
            acrtrunc = pacr[0:index[crossing]]
        else:
            acrtrunc = pacr
        tau = np.sum(acrtrunc)*dt*slice
        Dhummer = np.var(traj[0] - np.mean(traj[0]))*math.pow(10,-16)/tau
        DhummerArray[i] = Dhummer
    Dhummer = np.mean(DhummerArray)
    out_q.put("%0.2f %0.4e"%(args[2], Dhummer))
...
def plot():
    ...
    plt.savefig('Figures/lengthHummer.png', dpi=1200, transparent=True)
    plt.savefig('Figures/lengthHummer.eps', dpi=1200)
    plt.show()
